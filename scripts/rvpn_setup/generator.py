...
from .crypto import generate_hex_secret, generate_keypair, issue_certificate
from .models import ClientOptions, PeerSetup, SecurityPolicy, SetupConfig
from .prompts import (
...
def ask_security() -> SecurityPolicy:
    ...
    retry_jitter_ms = prompt_int(
        "Handshake retry jitter (ms)",
        150,
        minimum=0,
        maximum=86_400_000,
    )
    rekey_packet_limit = prompt_int(
        "Rekey packet limit (0 disables)",
        1 << 20,
        minimum=0,
    )
    rekey_time_limit_secs = prompt_int(
        "Rekey time limit (seconds, 0 disables)",
        120,
        minimum=0,
    )
    ...
    while True:
        liveness_timeout_secs = prompt_int(
            "Liveness timeout (seconds, 0 disables; otherwise >= 60)",
            90,
            minimum=0,
        )
        if liveness_timeout_secs == 0 or liveness_timeout_secs >= 60:
            break
        print("  RVPN requires liveness.timeout_secs to be 0 or at least 60.")
...
def ask_server() -> SetupConfig:
    ...
    server_interface_name = prompt_interface_name(
    ...
    if server_mode == "both":
        server_tap_name = prompt_interface_name(
            "Server TAP interface name",
            "rvpn-server0-tap",
        )

    server_mtu = prompt_int(
        "Server interface MTU",
        1400,
        minimum=576,
        maximum=65535,
    )
    server_v4 = prompt_network(
        "Server interface IPv4 address (CIDR)",
        "10.42.0.1/24",
    )
    ipv6_enabled = prompt_bool("Enable IPv6", True)
    server_v6 = ""
    if ipv6_enabled:
        server_v6 = prompt_network(
            "Server interface IPv6 address (CIDR)",
            "fd42::1/64",
        )

    print(f"  -> IPv4 tunnel CIDR: {network_cidr(server_v4)}")
    if ipv6_enabled:
        print(f"  -> IPv6 tunnel CIDR: {network_cidr(server_v6)}")

    forwarding_enabled = prompt_bool(
        "Enable internet gateway / NAT forwarding",
        True,
    )
    ...
    obfuscation_enabled = prompt_bool(
    ...
    if obfuscation_enabled:
        print(
            "  -> generated a new 32-byte obfuscation key for all generated clients"
        )

    auth_mode = prompt_choice(
        "Peer authentication mode",
        ["pinned-key", "certificate", "psk"],
        "pinned-key",
    )
...
def ask_client_options(
    setup: SetupConfig,
    peer_name: str,
    platform: str,
) -> ClientOptions:
    print(f"\n  -- {peer_name}: client interface / routing --")

    default_name = f"rvpn-{platform}-{slugify(peer_name)}"[:15]
    interface_name = prompt_interface_name(
    ...
    if platform == "android":
        mode = "tun"
        print("  Android clients are generated as TUN-only.")
    else:
        mode = prompt_choice(
    ...
    if mode == "both":
        tap_name = prompt_interface_name(
            "Client TAP interface name",
            f"{interface_name[:10]}-tap",
        )
    ...
    dns_default = derive_dns_from_endpoint_external(
        setup.endpoint_external
    )
    dns_servers = prompt_dns_servers(
        "Client DNS server(s)",
        dns_default,
    )
    ...
    gateway_v6 = (
        setup.server_v6.split("/")[0]
        if setup.ipv6_enabled
        else None
    )

    default_route = prompt_bool(
        "Route IPv4 default traffic through RVPN",
        True,
    )
    gateway = None
    if default_route:
    ...
    if setup.ipv6_enabled:
        default_route_v6 = prompt_bool(
            "Route IPv6 default traffic through RVPN",
            True,
        )
        if default_route_v6:
            gateway_v6_value = prompt_ip(
                "IPv6 tunnel gateway",
                gateway_v6 or "fd42::1",
            )
...
def gather_peers(setup: SetupConfig) -> list[PeerSetup]:
    pc_count = prompt_int(
        "How many PC peers to generate",
        0,
        minimum=0,
    )
    android_count = prompt_int(
        "How many Android peers to generate",
        1,
        minimum=0,
    )
    ...
    for global_index, (platform, platform_index) in enumerate(
        peer_specs,
        start=1,
    ):
        print(
        ...
        while True:
            ...
            if name in names:
                print("  peer names must be unique.")
                continue
            names.add(name)
            break

        tunnel_v4 = host_at_offset(
            setup.server_v4,
            global_index + 1,
        )
        tunnel_v6 = (
        ...
        print(f"  -> assigned IPv4: {tunnel_v4}")
        if setup.ipv6_enabled:
            print(f"  -> assigned IPv6: {tunnel_v6}")
...
def gather_links(
    ...
    count = prompt_int(
        "How many peer-to-peer link groups",
        0,
        minimum=0,
    )
    if count == 0:
    ...
    for index, peer in enumerate(peers, start=1):
        print(
            f"  {index}. {peer.name} -> {labels[index - 1]}"
        )
    ...
    for group_index in range(1, count + 1):
        while True:
            indexes = prompt_index_group(
                f"Link group {group_index} members (peer numbers)",
                len(peers),
            )
            group = [
                labels[index - 1]
            ...
            if identity in seen:
                print("  that link group is already configured.")
                continue
            seen.add(identity)
            groups.append(group)
            break
